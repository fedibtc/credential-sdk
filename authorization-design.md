...
the trust badge id when creating an authorization; they should pass the
selected `SignedCredential` value to the holder context and let the SDK derive
...
- `HolderAuthorizationRequest.credential` is the holder's selected signed
  credential. The SDK derives the signed `TrustBadgeId` from this credential.
- `HolderAuthorizationRequest.expires_at` is selected by the wallet. The SDK
  sets `issued_at` when signing.
...
- `subject_pubkey` is the external application's public key. It should use the
  ...
  multi-key support.
- `trust_badge_id` is not a free-form badge name. It is the
...
- `issued_at` and `expires_at` are Unix timestamps in seconds. Rust exposes
  these as `Timestamp`; TypeScript exposes `Timestamp` as a number alias.
- `authorization_id` is a future-proof application-chosen identifier. The MVP
  ...
  tracking, or revocation. MVP authorization creation sets this to an empty
  string.
...
string. It is exposed in the returned wire authorization for verifiers, not as
standalone WASM digest plumbing for wallet applications.
...
- The credential verifies against trusted issuer authorities and known
  revocations.
- The credential digest matches `authorization.trust_badge_id`.
...
3. Verify the holder authorization Schnorr proof with
   `authorization.holder_id_pubkey`.
4. Check authorization `issued_at` and `expires_at`.
5. Check the computed credential digest matches a signed trust badge id.
6. Extract the holder public key from `credential.blind_msg` and check it equals
   `authorization.holder_id_pubkey`.
7. Preserve `authorization_id` but do not apply MVP verifier policy to it.
8. Apply local policy to issuer, credential schema and `info`, holder, subject,
   and freshness.
