...
`CredentialRef` values when creating an authorization; they should pass the
selected `SignedCredential` values to the holder context and let the SDK derive
...
- `HolderAuthorizationRequest.credentials` are the holder's selected signed
  credentials. The SDK derives each `CredentialRef` from these credentials.
...
- `credential_refs[].trust_badge_id` is not a free-form badge name. It is the
  ...
  `Revocation.credential_digest`. This is generated by the SDK during
  authorization creation.
- `scope` is a future-proof field. The only defined MVP value is `present`; MVP
  authorization creation sets `scope` to `["present"]`, and MVP verification
  signs and preserves this field but does not apply scope-specific policy.
...
6. Extract the holder public key from `credential.blind_msg` and check it equals
   `authorization.holder_id_pubkey`.
7. Preserve `scope` and `authorization_id` but do not apply MVP verifier policy
   to them.
