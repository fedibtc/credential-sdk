...
sequenceDiagram
  ...
  W->>W: Select SignedCredential values and expires_at
...
credential refs from selected `SignedCredential` values. The verifier-side SDK
check verifies the credential, holder authorization signature, holder binding,
credential reference, and authorization time window. Subject-key
